...
class TelegramRequests(object):
    ...
    def make_request(self, method_: str, endpoint: str, params: dict[str, Any] = None) -> Union[dict[str, Any], None]:
        ...
        """
        self.uri_to_log: str = f"{self.route['uri']}[token]{endpoint}"
        try:
            response: requests.models.Response = requests.request(method_, f"{self.uri}{endpoint}", params=params)
            return response.json()
        except requests.exceptions.HTTPError as errh:
            log_connect.report(method_, self.uri_to_log, "error", "INTERNAL", False, "HTTP Error")
            pass
        except requests.exceptions.ProxyError as errp:
            log_connect.report(method_, self.uri_to_log, "error", "INTERNAL", False, "Proxy Error")
            pass
        except requests.exceptions.ConnectionError as errc:
            log_connect.report(method_, self.uri_to_log, "error", "INTERNAL", False, "Error Connecting")
            pass
        except requests.exceptions.Timeout as errt:
            log_connect.report(method_, self.uri_to_log, "error", "INTERNAL", False, "Timeout Error")
            pass
        except requests.exceptions.RequestException as err:
            log_connect.report(method_, self.uri_to_log, "error", "INTERNAL", False, "Unknown Error")
            pass
